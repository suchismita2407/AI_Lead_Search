...
export interface WorkspaceAccess { allowed: boolean; admin: boolean; status: "admin" | "trial" | "active" | "expired"; trialEndsAt: string | null; leadLimit: number | null; }
...
const TRIAL_LEAD_LIMIT = 10;
function configuredAdminEmail() { return (process.env.ADMIN_EMAIL ?? "").trim().toLowerCase(); }
async function ensureTrial(userId: number) {
  const [existing] = await sql()`SELECT id FROM subscriptions WHERE user_id = ${userId}`;
  if (existing) return;
  const started = nowIso();
  await sql().insert`INSERT INTO subscriptions (user_id, provider, plan, status, trial_started_at, trial_ends_at, current_period_end) VALUES (${userId}, 'internal', 'trial', 'trialing', ${started}, ${expiresIso(TRIAL_SECONDS)}, ${expiresIso(TRIAL_SECONDS)})`;
}
export async function getWorkspaceAccessForUser(user: SessionUser): Promise<WorkspaceAccess> {
  if (configuredAdminEmail() && user.email.toLowerCase() === configuredAdminEmail()) return { allowed: true, admin: true, status: "admin", trialEndsAt: null, leadLimit: null };
  await ensureTrial(user.id);
  const [sub] = await sql()`SELECT plan, status, trial_ends_at, current_period_end FROM subscriptions WHERE user_id = ${user.id}`;
  const status = String(sub?.status ?? "inactive");
  const plan = String(sub?.plan ?? "free");
  const trialEndsAt = sub?.trial_ends_at == null ? null : String(sub.trial_ends_at);
  if (status === "active" && plan !== "free" && plan !== "trial") return { allowed: true, admin: false, status: "active", trialEndsAt: null, leadLimit: null };
  if (status === "trialing" && trialEndsAt && new Date(trialEndsAt).getTime() > Date.now()) return { allowed: true, admin: false, status: "trial", trialEndsAt, leadLimit: TRIAL_LEAD_LIMIT };
  if (status === "trialing") await sql().run`UPDATE subscriptions SET status = 'expired', updated_at = ${nowIso()} WHERE user_id = ${user.id}`;
  return { allowed: false, admin: false, status: "expired", trialEndsAt, leadLimit: 0 };
}
export async function requireWorkspaceAccess(): Promise<SessionUser> { const user = await requireUser(); const access = await getWorkspaceAccessForUser(user); if (!access.allowed) throw redirect({ to: "/pricing", search: { expired: "1" } }); return user; }
